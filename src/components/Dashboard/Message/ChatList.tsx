    ...
    <div className="flex flex-col w-[390px] h-full">
      <div className="h-[15vh]">
        ...
        <div className="mt-4 w-full h-[60px] flex justify-between items-center px-5 bg-input">
          <h1 className="bold-2 text-contrast-100">All {seller ? "Clients" : "Sellers"}</h1>
          <div className="bg-white rounded-full px-4 py-[6px] text-[12px] leading-[24px] font-normal text-contrast-base">
            {conversations.length}
          </div>
        </div>
      </div>
      <div className="flex flex-col overflow-y-scroll h-[75vh] scrollbar-custom">
        {conversations.map((conversation, i) => {
          return (
            <div
              ...
              className={`flex items-center w-full pl-1 pr-5 gap-3 py-2 cursor-pointer ${i === currentMessage && "bg-blue-10 rounded border-[0.5px] border-y-contrast-10 border-x-0"}`}
            >
